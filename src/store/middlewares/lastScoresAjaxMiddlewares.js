...
import { GET_LAST_SCORE_NBA, GET_LAST_SCORE_NFL, GET_LAST_SCORE_NHL, GET_LAST_SCORE_MLB, GET_LAST_SCORE_MLS } from '../actionsTypes';
import { getLastScroresNBAsuccess, getLastScroresMLBsuccess, getLastScroresNFLsuccess, getLastScroresNHLsuccess, getLastScroresMLSsuccess, getLastScroresError} from '../actionLastScores';

const lastScoresAjaxMiddleware = (store) => (next) => (action) => {
    next(action);
    switch (action.type) {
        case GET_LAST_SCORE_NBA:
            console.log('type:', GET_LAST_SCORE_NBA);
            axios({
                method: 'get',
